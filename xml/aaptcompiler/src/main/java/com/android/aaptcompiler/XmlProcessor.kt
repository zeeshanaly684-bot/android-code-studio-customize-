...
class XmlProcessor(val source: Source, val logger: BlameLogger?) {
  ...
  fun process(file: ResourceFile, inputFile: InputStream) {
    ...
    try {
      ...
      processElement(rootStart.asStartElement(), eventReader, collectedIds, protoBuilders, builder)
   ...
   *
   * This method flattens the XML tree whose root is [startElement], aggregates all created ids, and
   * outlines any given aapt:attr xml subtrees that may exist in the given XML.
   *
   * @param startElement the start element that represents the root of the XML tree to be flattened.
   *   The start element should not be pointing to a <aapt:attr> element, as those are processed in
   *   a different method.
   * @param eventReader the stream from which the XML file is read. The event reader should be in a
   *   state where [startElement] was the last event read. When this method finishes, [eventReader]
   ...
   * @param resourceBuilders The map of all XML resource builders created for this XML document. Any
   *   new builders created because of <aapt:attr> subelements of [startElement] will be added to
   *   this map.
   ...
   * @param eventReader the stream from which the XML file is read. The event reader should be in a
   *   state where [attrElement] was the last event read. When this method finishes, [eventReader]
   *   will be positioned immediately after the [EndElement] that aligns with the supplied
   *   [attrElement].
   * @param collectedIds The map of all created ID resources in the XML document. Any created ids
   *   found in the XML tree represented by [attrElement] will be added to this map.
   * @param resourceBuilders The map of all XML resource builders created for this XML document. At
   *   least one new builder will be added from the outlined aapt:attr (if successful), or more if
   *   there are nested aapt:attr definitions.
   ...
   *
   * @param startElement the start element that represents the root of the XML file to be extracted.
   * @param eventReader the stream from which the XML file is read. The event reader should be in a
   *   state where [startElement] was the last event read. When this method finishes, [eventReader]
   *   will be positioned immediately after the [EndElement] that aligns with the supplied
   *   [StartElement].
   ...
   * @return the ResourceFile corresponding to the outlined element. This value will be null if the
   *   outlining is unsuccessful.
   */
